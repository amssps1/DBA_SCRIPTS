...
-----------------------------------------------------------------------
DECLARE @BackupHoje        VARCHAR(500) = 'H:\Backup_Dia';
DECLARE @BackupRetencao    VARCHAR(500) = 'H:\Backup_Retencao';
...
EXEC xp_cmdshell 'for /R "H:\Backup_Dia" %i in (*.bak *.trn) do @echo %i';
...
SELECT name
FROM sys.databases
WHERE name NOT IN ('tempdb', 'bd_temp','Bd_temp_ccm','DW_Auto','InfanteSagres')

--WHERE name NOT IN ('tempdb', 'Almonde', 'DM_ACM','dba_db','DM_ACM','DM_Actividade','AdministracaoSistemas','DM_ADEPTRA','DM_CS','DM_DAH','DM_DG','DM_Fraude','DM_GE','DM_IDH','DM_IPCC')
