/****************************************************************************************
Script de criação Login de Deploy: Release_Deploy_Login.sql
Versão          : 1.1
Autor           : António Santos Silva
Data Criação    : 2025-05-13
Descrição       : Criação automático de Login para TFS - Release deploy 

Histórico de Alterações:
---------------------------------------------------------------------------------------------------------------------------
| Versão | Data       | Autor             | Descrição                                									   |
|--------|------------|-------------------|--------------------------------------------------------------------------------|
| 1.0    | 2025-05-12 | António silva     | Criação inicial da script                  									   |
| 1.1    | 2025-05-15 | António silva     | Adicionado o privilégio do Login poder criar Schemas em cada Base de Dados     |
| 
-----------------------------------------------------------------------------------------------------------------------------
...
-- Vamos retirar o privilégio de criação de Base de Dados, este processo deve ser melhor controlado do que actualmente, e deixar de ser automatizado
-- SET @SQL = N'GRANT CREATE ANY DATABASE TO [' + @LoginName + '];';
-- EXEC sp_executesql @SQL;


-- 2. Conceder permissões no nível do servidor para visualizar a definição de objectos em qualquer base de dados 

SET @SQL = N'GRANT VIEW ANY DEFINITION TO [' + @LoginName + '];';
EXEC sp_executesql @SQL;


-- 3. Processar cada BD de utilizador e que esteja online
...
BEGIN
    ...
    END TRY
    BEGIN CATCH
        PRINT 'Erro ao criar Login na BD: ' + @DBName + ': ' + ERROR_MESSAGE();
    END CATCH
	...
	GRANT CREATE SCHEMA TO [' + @LoginName + '];';
    
    BEGIN TRY
        EXEC sp_executesql @SQL;
    END TRY
    BEGIN CATCH
        PRINT 'Erro ao conceder permissões à Base de Dados ' + @DBName + ': ' + ERROR_MESSAGE();
    END CATCH
    ...
    BEGIN
        ...
        SET @DynamicSQL = ''GRANT SELECT, INSERT, UPDATE, ALTER ON SCHEMA::'' + QUOTENAME(@SchemaName) + '' TO [' + @LoginName + ']'';
    ...
    END TRY
    BEGIN CATCH
        PRINT 'Erro ao processar schemas na BD: ' + @DBName + ': ' + ERROR_MESSAGE();
    END CATCH
...
-- 5. Processar SSISDB separadamente - SOMENTE SE EXISTIR SSIS Catalog na instancia
IF @SSISDBExists = 1
BEGIN
    SET @SQL = N'
    USE [SSISDB];
...
PRINT 'Processamento finalizado para o Deploy do login: ' + @LoginName;